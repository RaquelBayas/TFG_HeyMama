...
import app.example.heymama.databinding.ActivitySettingsBinding
...
class SettingsActivity : AppCompatActivity() {
            ...
            builder.setPositiveButton("Confirmar") { _: DialogInterface, which ->
                if (password.text.isEmpty()) {
                    ...
                    val credential: AuthCredential =
                        EmailAuthProvider.getCredential(currentUser.email!!,
                    ...
                    currentUser.reauthenticate(credential).addOnCompleteListener { task ->
                        if (task.isSuccessful) {
                            ...
                            deleteForos(uid)
                            deleteUserChats(uid)
                            database.reference.child("Usuarios").child(uid).child("rol").get()
                            ...
                            firestore.collection("Usuarios").document(uid).delete()
                                .addOnSuccessListener {
                                    Log.i("Firestore account", "OK")
                                }.addOnFailureListener {
                                Log.e("Firestore accound", it.toString())
                            }
    ...
    /**
     * Este método permite al usuario cambiar su nombre
     */
